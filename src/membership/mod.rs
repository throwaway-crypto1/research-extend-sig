...
use cpsnarks_set::commitments::{pedersen::PedersenCommitment, Commitment};
...
impl<R1: MutRandState, R2: RngCore + CryptoRng> SetMembership<R1, R2> {
    pub fn commit_to_set_element(
        &self,
        value: &Integer,
        randomness: &ElemCommRandomness,
    ) -> ElemCommitment {
        self.protocol
            .crs
            .crs_modeq
...
pub mod tests {
    ...
    pub fn random_xw<R1: MutRandState, R2: RngCore + CryptoRng>(
        setmem: &mut SetMembership<R1, R2>,
    ) -> (SetMemStatement, SetMemWitness) {
